...
const FilterModal = ({selectedFilters,onFilterChange,onClose}) => {
    ...
    const[propertyType,setPropertyType]=useState(
        selectedFilters.propertyType || "" //default it is empty or the selected property from the props
    );
    ...
    //useEffect hook to update states when selected filters change
    useEffect(()=>{
        setPriceRange({
            min: selectedFilters.priceRange?.min || 600,
            max: selectedFilters.priceRange?.max || 30000,
    ...
    //function to handle min value of price range
    ...
    //function to handle max value of price range
...
const handleAmenitiesChanges=(selectedAmenity)=>{
    setAmenities((prevAmenities)=>prevAmenities.includes(selectedAmenity)?prevAmenities.filter((item)=>item!==
    selectedAmenity):[...prevAmenities,selectedAmenity]
    );
};
...
const handleRoomTypeChange=(selectedType)=>{
 ...
 <div className='filter-section'>
        ...
        <div className='amenities-checkboxes'>
            ...
            onChange={()=>handleAmenitiesChanges(options.value)}
            />
